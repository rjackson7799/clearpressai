/**
 * Deno-side duplicate of `src/lib/prompts/compliance.ts`.
 *
 * Drift-guarded regions are byte-equality tested by
 * `src/lib/prompts/compliance.sync.test.ts` (H10 audit fix — full-function-
 * body comparison, not just static template fragments).
 *
 * CLAUDE_MODELS is duplicated from `src/lib/prompts/brand-voice.ts`. Keep all
 * four pins identical when bumping; the brand-voice + variant-generation sync
 * tests already enforce this for those copies.
 */
